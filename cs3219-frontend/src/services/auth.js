...
export const verifyTokenService = async () => {
  try {
    return await axios.post(`${API_URL}/users/auth/verifyToken`);
  } catch (err) {
    return {
      error: true,
...
export const userLoginService = async (username, password) => {
  try {
    return await axios.post(`${API_URL}/users/auth/signin`, {
      username,
      password,
    });
  } catch (err) {
...
export const userLogoutService = async () => {
  try {
    return await axios.post(`${API_URL}/users/auth/logout`);
  } catch (err) {
    return {
      error: true,
      response: err.response,
